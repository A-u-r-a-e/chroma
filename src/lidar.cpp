...
void run_dresets(ms pollrate) {
    while (comp_state != CompState::DISABLE) {
        ...
        master.print(0, 0, "x: %.1f y: %f.1f h: %.2f", pose.pos.x, pose.pos.y, to_deg(pose.dir));
        front_lidar.update();
        left_lidar.update();
        right_lidar.update();
        delay_for(pollrate);
...
bool side_reset(Quad quadrant) {
    ...
    double m_l = left_lidar.find_norm_distance(offset_angle);
    double m_r = right_lidar.find_norm_distance(offset_angle);

    pros::lcd::print(7, "tdist, l: %f, r: %f", m_l, m_r);

    if (check_collision(left_pose)) m_l = -1.0;
    if (check_collision(right_pose)) m_r = -1.0;
...
bool fwd_reset(Quad quadrant, bool calculate_heading) {
    ...
    Pose l_pose = Pose{reset_pos + rotate(Vec{0, front_lidar.get_left_offset()}, angle_rad)};
    Pose r_pose = Pose{reset_pos + rotate(Vec{0, -front_lidar.get_right_offset()}, angle_rad)};
    ...
    pros::lcd::print(6, "fwd true dist: %f", measurement);
    ...
    localizer->set_pose(Pose{reset_pos, to_rad(reset_angle)});
